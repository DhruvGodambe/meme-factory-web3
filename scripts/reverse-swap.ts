...
const SELL_TOKEN_OVERRIDE =
  process.env.SELL_TOKEN_OVERRIDE && process.env.SELL_TOKEN_OVERRIDE !== ""
    ? process.env.SELL_TOKEN_OVERRIDE
    : "0xBC599f4C256ab3B76aC7b8949452f6ABbB27A69a"; // override rarity token for sell path
...
async function runSwapFeeVerification({
  ...
  const rarityFromFactory = await factory.collectionToNFTStrategy(collectionAddress);
  const rarityToken =
    SELL_TOKEN_OVERRIDE && SELL_TOKEN_OVERRIDE !== ""
      ? SELL_TOKEN_OVERRIDE
      : rarityFromFactory;

  if (rarityToken === ethers.ZeroAddress) {
    console.log(`  ⚠️ No RARITY token found (resolved address is zero)\n`);
    return;
  }

  console.log(`  RARITY Token: ${rarityToken}`);
  if (SELL_TOKEN_OVERRIDE && SELL_TOKEN_OVERRIDE !== "") {
    console.log(`  (Using override token address)`);
  }

  const deployerAddress = await deployer.getAddress();
  const routerAddress = (router as any).target ?? (router as any).address ?? "";
  const poolManagerAddress = await hook.manager();
  const token = new ethers.Contract(rarityToken, ERC20_ABI, deployer);
  const tokenBalance = await token.balanceOf(deployerAddress);

  if (tokenBalance === 0n) {
    console.log(`  ⚠️ No balance of RARITY token in wallet ${deployerAddress}\n`);
    return;
  }

  const swapAmount = tokenBalance;
  console.log(`  Swap Amount (token in): ${ethers.formatEther(swapAmount)} (full wallet balance)`);
  console.log("  Direction: RARITY -> Base ETH (zeroForOne = false)");
  ...
  trackedAddresses.swapper = deployerAddress;
  ...
  console.log("  ➤ Ensuring router is allowlisted in factory...");
  try {
    await ensureRouterAllowlisted(factory, routerAddress);
    console.log("    ✅ Router allowlisted");
  } catch (err: any) {
    console.error(`    ❌ Failed to ensure router allowlist: ${err?.message ?? err}`);
    console.log("    Router not allowlisted will cause transfer revert (EOA -> router).");
    console.log();
    return;
  }

  console.log("  ➤ Ensuring pool manager is allowlisted in factory...");
  try {
    await ensureRouterAllowlisted(factory, poolManagerAddress);
    console.log("    ✅ Pool manager allowlisted");
  } catch (err: any) {
    console.error(`    ❌ Failed to ensure pool manager allowlist: ${err?.message ?? err}`);
    console.log("    Pool manager not allowlisted will cause transfer revert (EOA -> poolManager).");
    console.log();
    return;
  }

  console.log("  ➤ Ensuring router has token allowance...");
  try {
    await ensureTokenApproval(rarityToken, deployer, routerAddress, swapAmount);
    console.log("    ✅ Allowance OK");
  } catch (err: any) {
    console.error(`    ❌ Failed to approve token: ${err?.message ?? err}`);
    console.log();
    return;
  }
  ...
  try {
    const tx = await router.swapExactTokensForTokens(
      ...
      deployerAddress,
      deadline,
      { gasLimit: 10_000_000n, ...gasOverrides }
    );
...
async function ensureTokenApproval(tokenAddress: string, owner: Signer, spender: string, amount: bigint) {
  if (!spender) throw new Error("router address missing");
  ...
  console.log(
    `    Approving ${spender} to spend ${ethers.formatEther(amount)} tokens on behalf of ${ownerAddress}...`
  );
  const tx = await token.approve(spender, amount);
  await tx.wait();
}

async function ensureRouterAllowlisted(factory: Contract, routerAddress: string) {
  if (!routerAddress) throw new Error("router address missing");
  const isAllowlisted = await (factory as any).listOfRouters(routerAddress);
  if (isAllowlisted) return;
  const signer = factory.runner as Signer;
  if (!signer) throw new Error("factory signer missing");
  const owner = await (factory as any).owner?.();
  const caller = await signer.getAddress();
  if (owner && owner.toLowerCase() !== caller.toLowerCase()) {
    throw new Error(`router not allowlisted and caller is not factory owner (${caller} vs ${owner})`);
  }
  console.log(`    Router ${routerAddress} not allowlisted — calling setRouter(...) as owner`);
  const tx = await (factory as any).setRouter(routerAddress, true);
  await tx.wait();
}
